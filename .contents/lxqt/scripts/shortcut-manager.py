...
def parse_desktop_file(filepath):
    name, icon, custom = "", "application-x-executable", False
    try:
...
class ShortcutApp(QMainWindow):
    ...
    def init_creator_tab(self):
        ...
        self.intel_group = QGroupBox("Inteligentní balení (jen pro nové)")
        ...
        self.category_input.setIconSize(QSize(24, 24))
        for friendly_name, xdg_name, icon_name in XDG_CATEGORIES:
            icon = QIcon.fromTheme(icon_name)
            if icon.isNull(): icon = QIcon.fromTheme("applications-other")
    ...
    def init_manager_tab(self):
        ...
        self.search_bar.setPlaceholderText("Hledat v menu...")
        ...
        self.delete_btn.setStyleSheet("""
            QPushButton { background-color: #f44336; color: white; padding: 10px; font-weight: bold; border-radius: 4px; }
            QPushButton:disabled { background-color: #e0e0e0; color: #a0a0a0; border: 1px solid #cccccc; }
        """)
        self.delete_btn.setEnabled(False) 
        
        button_layout.addWidget(self.save_btn); button_layout.addWidget(self.edit_btn); button_layout.addWidget(self.delete_btn)
    ...
    def load_applications(self):
        self.table.setRowCount(0); apps_dict = {} 
        ...
        for root, dirs, files in os.walk(APPS_DIR):
            for f in files:
                if f.endswith(".desktop"):
                    ...
                    typ = "Vlastní" if is_custom else ("Wine Aplikace" if rel_path.startswith(f"wine{os.sep}") else "Systémový")
                    apps_dict[f] = {"name": name, "icon": icon, "path": path, "typ": typ}
        ...
        for row, key in enumerate(sorted_keys):
            data = apps_dict[key]
            ck_widget = QWidget(); ck_layout = QHBoxLayout(ck_widget); cb = QCheckBox()
            try:
                with open(data["path"], 'r', errors='ignore') as f: cb.setChecked("NoDisplay=true" not in f.read())
            except: pass
            if "správce zástupců" in data["name"].lower(): cb.setEnabled(False); cb.setChecked(True)
            ...
            item = QTableWidgetItem(data["name"]); item.setIcon(get_app_icon(data["icon"]))
            item.setFlags(item.flags() & ~Qt.ItemIsEditable); self.table.setItem(row, 1, item)
            typ_item = QTableWidgetItem(data["typ"]); typ_item.setFlags(typ_item.flags() & ~Qt.ItemIsEditable)
            if data["typ"] == "Vlastní": typ_item.setForeground(Qt.blue)
            elif data["typ"] == "Wine Aplikace": typ_item.setForeground(Qt.darkMagenta)
            self.table.setItem(row, 2, typ_item); self.table.setItem(row, 3, QTableWidgetItem(data["path"]))

    def check_row_selection(self):
        row = self.table.currentRow()
        if row < 0:
            self.delete_btn.setEnabled(False); self.edit_btn.setEnabled(False); return
    ...
    def edit_shortcut(self):
        row = self.table.currentRow(); path = self.table.item(row, 3).text()
        ...
        # SUROVÉ ZOBRAZENÍ CESTY
        self.name_input.setText(name); self.exec_input.setText(exec_cmd)
        self.icon_input.setText(icon); self.terminal_cb.setChecked(terminal)
        idx = self.category_input.findData(category)
        if idx >= 0: self.category_input.setCurrentIndex(idx)
        self.editing_path = path; self.create_btn.setText("Uložit úpravy zástupce")
    ...
    def create_shortcut(self):
        ...
        # --- ZÁCHRANA PRACOVNÍ SLOŽKY (Path) ---
        # Nejdřív zkusíme najít první cestu v uvozovkách (kvůli Program Files)
        working_dir = ""
        import re
        matches = re.findall(r'"(/[^"]+)"', exec_path)
        # Hledáme v uvozovkách soubor, který končí na .exe
        for m in matches:
            if m.lower().endswith('.exe'):
                working_dir = os.path.dirname(m)
                break
        
        # Pokud jsme nic nenašli v uvozovkách, zkusíme starou metodu
        if not working_dir:
            clean_exec = exec_path.strip('"\'')
        ...
        if hasattr(self, 'editing_path') and self.editing_path:
            ...
            final_exec = exec_path
            if exec_path.lower().endswith('.exe') and not exec_path.startswith("wine "):
                final_exec = f"wine \"{exec_path}\""
            if self.wrapper_cb.isChecked() and f"python3 \"{BUSY_SCRIPT}\"" not in final_exec:
        ...
        try:
            ...
            with open(output_file, 'w') as f:
                ...
                # ZÁPIS PATH - Teď už je venku z podmínek a zapíše se vždycky!
            ...
            self.editing_path = None; self.create_btn.setText("Vytvořit zástupce v Menu")
            for i in [self.name_input, self.exec_input, self.icon_input]: i.clear()
        except Exception as e: QMessageBox.critical(self, "Chyba", str(e))

    def filter_apps(self, text):
        for i in range(self.table.rowCount()):
            name = self.table.item(i, 1).text().lower(); self.table.setRowHidden(i, text.lower() not in name)
        ...
        for i in range(self.table.rowCount()):
            ...
            try:
                ...
                with open(path, 'w') as f:
                    ...
                    if not widget.isChecked(): f.write("NoDisplay=true\n")
            except: continue
        self.refresh_system_menu(); QMessageBox.information(self, "Hotovo", "Změny uloženy.")

    def delete_shortcut(self):
        row = self.table.currentRow(); path = self.table.item(row, 3).text()
        if QMessageBox.question(self, "Smazat", "Opravdu smazat?") == QMessageBox.Yes:
            try: os.remove(path); self.load_applications(); self.refresh_system_menu()
            except: pass
    ...
    def validate_exec_intelligence(self, text):
        ...
        self.intel_group.show(); suffix = QFileInfo(path).suffix().lower()
        if suffix == 'exe':
    ...
    def extract_exe_icon(self):
        ...
        if not shutil.which("wrestool"):
            QMessageBox.critical(self, "Chyba", "Nainstaluj icoutils!"); return
        info = QFileInfo(exe_path); tmp_ico = f"/tmp/{info.baseName()}.ico"
        try:
            ...
            if os.path.exists(tmp_ico) and os.path.getsize(tmp_ico) > 0:
                out_dir = f"/tmp/{info.baseName()}_png"; os.makedirs(out_dir, exist_ok=True)
                subprocess.run(["icotool", "-x", tmp_ico, "-o", out_dir])
                pngs = glob.glob(f"{out_dir}/*.png")
                if pngs:
                    best = max(pngs, key=os.path.getsize); target = os.path.join(info.absolutePath(), f"{info.baseName()}.png")
...
if __name__ == "__main__":
    ...
    QIcon.setThemeName("Papirus"); window = ShortcutApp(); window.show()
    sys.exit(app.exec_())